...
def verify_html(html_path, viewports=None, slides=0, output_dir=None, show=False, wait=2000, clicks=None):
    try:
    ...
    with sync_playwright() as p:
        ...
        for viewport in viewports:
            ...
            if slides > 0:
                ...
                if clicks:
                    print(f"  → 运行点击流，共 {len(clicks)} 步")
                    for idx, selector in enumerate(clicks, start=1):
                        try:
                            locator = page.locator(selector).first
                            locator.wait_for(state="visible", timeout=5000)
                            locator.click()
                            page.wait_for_timeout(400)
                            click_path = output_dir / f"{stem}{suffix}-click-{str(idx).zfill(2)}.png"
                            page.screenshot(path=str(click_path), full_page=False)
                            print(f"    ✓ click {idx}: {selector} → {click_path.name}")
                        except Exception as exc:
                            message = f"click {idx} failed for selector `{selector}`: {exc}"
                            click_failures.append(message)
                            print(f"    ❌ {message}")
    ...
    return 0 if not page_errors and not click_failures else 1
...
def main():
    ...
    parser.add_argument("--click", action="append", default=[],
                        help="按顺序点击的 CSS selector，可重复传入")
